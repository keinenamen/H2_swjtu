...
public class RegisterActivity extends FragmentActivity {
    ...
    private void initView() {
        ((TextView)findViewById(R.id.tv_head_title)).setText("用户注册");
...
//                            public void run() {
                                try {
                                    boolean mp =
                                            DBUtils.insertUserByName(useName,psd);
                                    Message msg = new Message();
                                    if (mp == false){
                                        msg.what = 0;
                                        Log.d("TAG", " 用户创建失败，请稍候再试！");
                                    }
                                    else{
                                        Tools.showProgDlg(RegisterActivity.this);
                                        GloableData.mApp.setUuid(useName);
                                        msg.what = 1;
                                        Log.d("TAG", " 用户创建成功");
                                        Intent regIntent = new Intent(RegisterActivity.this,
                                                LoginActivity.class);
...
//                        }).start();
    ...
    private void setListener() {
        ...
        findViewById(R.id.btn_registeractivity_sure).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {//
                String useName = et_tel.getText().toString().trim();
